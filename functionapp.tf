// Administra una aplicación de función, al final con aplicaciones de consola
resource "azurerm_function_app" "funct_app_sistcont" {
    
    name                      = "function-sistcont-${var.project}-${var.environment}"
    ...
    site_config {
        ...
        ip_restriction { //No permitir ninguna conexion que venga por internet
            name      = "default-deny"
    ...
    app_settings = {
        ...
        "AzureWebJobsDashboard"       = azurerm_storage_account.storage_account.primary_connection_string // Estos dos son metricas que se generan del uso de las functions apps
        "WEBSITE_VNET_ROUTE_ALL"      = "1"
        "QueueStorageConnectionString" = azurerm_storage_account.storage_account.primary_connection_string // Manejar la conexion al queue storage
...
resource "azurerm_private_endpoint" "function_private_endpoint"{
    ...
    private_service_connection {
        ...
        private_connection_resource_id = azurerm_function_app.funct_app_sistcont.id
